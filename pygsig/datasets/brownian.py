...
#--------------------------------------------------------------------------
# SIMULATION OF 1D BROWNIAN MOTION WITH SYNCHRONIZING PERIODIC DRIFT
#--------------------------------------------------------------------------
...
#--------------------------------------------------------------------------
# SIMULATION OF 3D BROWNIAN MOTION WITH SYNCHRONIZING PERIODIC DRIFT
#--------------------------------------------------------------------------
...
class Simulation3D():
    ...
    def run(self,graph_seed,omega_seed,param_seed):
        ...
        # Initialize 3D signals for each node
        X = np.random.rand(self.num_nodes, 3)  # 3D signal for each node

        # Simulate
        theta_traj = np.zeros((self.num_nodes, self.num_time_steps))
        mu_traj = np.zeros((self.num_nodes, self.num_time_steps))
        X_traj = np.zeros((self.num_nodes, self.num_time_steps, 3))  # 3D trajectory
        ...
        for step, t in enumerate(tt):
            ...
            theta = kuramoto(graph, theta, omega, self.dt)
            mu = periodic_drift(beta, theta, omega, mu_0, t)
            X = X + self.dt * mu[:, None] + np.sqrt(self.dt) * self.sigma_gain * np.random.randn(self.num_nodes, 3)  # Update in 3D
    ...
    def get_sequence(self):
        ...
        # Update features to store 3D vectors
        features = [torch.tensor(self.X[:, t, :], dtype=torch.float) for t in range(snapshot_count)]
        targets = [y for _ in range(snapshot_count)]
